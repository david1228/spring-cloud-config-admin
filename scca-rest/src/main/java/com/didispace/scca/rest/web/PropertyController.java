...
public class PropertyController extends BaseController {
    ...
    public WebResp<Map<String, Object>> propertiesFromPersistent(@RequestParam("project") String project,
                                                        @RequestParam("profile") String profile,
                                                        @RequestParam("label") String label) throws JsonProcessingException {
        // 通过配置中心来获取配置信息, 参考应用获取的具体配置信息
        Environment environment = baseOptService.getProperties(project, profile, label);
        Map<String, Object> properties = persistenceService.readProperties(environment, project, profile, label);
        // 直接通过持久化实现，获取配置信息
